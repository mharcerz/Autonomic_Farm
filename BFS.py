...
class Node:  # wierzchołek grafu
    def __init__(self, action, parent, x, y):
        self.action = action  # gdzie ma pójść (lewo/prawo/przód/tył)
...
def graphsearch(explored, fringe, goaltest, istate, succ):  # przeszukiwanie grafu wszerz
    node = Node(None, istate.get_x(), istate.get_y())  # wierzchołek początkowy, stworzony ze stanu początkowego traktora
...
def succ( elem):  # funkcja następnika, przypisuje jakie akcje są możliwe do wykonania na danym polu oraz jaki będzie stan (kierunek, położenie) po wykonaniu tej akcji
    ...
    if board.Cart.is_move_allowed_succ(elem) == "x + 1":
        actions_list.append(("move", (elem.get_direction(), temp_move_east, elem.get_y())))
    elif board.Cart.is_move_allowed_succ(elem) == "y - 1":
    ...
    elif board.Cart.is_move_allowed_succ(elem) == "x - 1":
